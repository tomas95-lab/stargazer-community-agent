...
create table if not exists public.ai_usage_events (
  ...
  status text not null check (status in ('success', 'error', 'blocked', 'reserved')),
  created_at timestamptz not null default now()
);

create index if not exists ai_usage_events_owner_idx on public.ai_usage_events(owner_id, created_at desc);
create index if not exists ai_usage_events_project_idx on public.ai_usage_events(project_key, created_at desc);
...
create table if not exists public.user_ai_keys (
  ...
  gemini_api_key_ciphertext text not null default '',
