...
export default function ReviewQueue() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      setResult(await api.getReviewQueue(150));
    } catch (err) {
  ...
  return (
    <div className="space-y-6 px-6">
      ...
      <div className="grid gap-3 md:grid-cols-5">
        <Stat label="All" value={result?.totals.all ?? '-'} />
      ...
      <section className="sg-panel overflow-hidden p-0">
        {loading && !result ? (
          ...
          <>
            ...
            {grouped.high.map((item) => <QueueItem key={item.id} item={item} />)}
            ...
            {grouped.other.map((item) => <QueueItem key={item.id} item={item} />)}
          </>
        ) : (
          <p className="p-5 text-sm text-muted-foreground">No human review items found in recent runs.</p>
        )}
      </section>
    </div>
  );
